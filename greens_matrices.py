...
def g_grid_meas_weight(
    ...
    LIUQE paper  transposed and multiplied by ΔR * Δz. It is transposed because G_COEF_MEAS_WEIGHT is stored
    ...
    also note that:
        ...
        g_pls_grid * G_GRID_MEAS_WEIGHT 
    
    in the RTGSFIT code, where G_COEF_MEAS_WEIGHT is stored in column major order.
    """
    ...
    for i, (r_g, z_g) in enumerate(zip(r_grid.flatten(), z_grid.flatten())):
        ...
        # The second one goes around the both the plasma and the vessel.
        if n_rog != 2:
            raise ValueError("n_rog must be 2, as we assume there are two Rogovski coils.")
        j += 2
        g_grid_meas_weight[i, j] = 1 * weights[j]
