"""
This module provides functions to calculate the mutual inductance between infinite straight wires.
...
def mutual_inductance_psi(
    ...
    z_prime: float) -> float:
    """
    Calculate the mutual inductance between two infinite straight wires.

    This function computes the mutual inductance between two infinite straight wires
    ...
    return -(MU_0 / (2 * np.pi)) * np.log(rho / dist)
...
def self_inductance_linear_avg(r: float, z: float, delta: float) -> float:
    ...
    where:
        - the lower limit of integration is R' = R - Δ/2,
        - the upper limit of integration is R' = R + Δ/2,
        - μ₀ is the permeability of free space,
    ...
    Returns:
        float: The averaged self-inductance in henries Tm/A.
    """

    dist = np.sqrt(r ** 2 + z ** 2)

    return -(MU_0 / (2 * np.pi)) * (np.log(0.5 * delta / dist) - 1)
...
def mutual_inductance_br(
    ...
    """
    Calculate the mutual inductance between two infinite straight wires based on the radial component
    of the magnetic field, given by ∂M/∂z.
    ...
    return -MU_0 / (2 * np.pi) * (z - z_prime) / ((r - r_prime) ** 2 + (z - z_prime) ** 2)
...
def mutual_inductance_bz(
    ...
    z_prime: float) -> np.ndarray:
    """
    Calculate the mutual inductance between two infinite straight wires based on the axial component
    of the magnetic field, given by -∂M/∂R.
    ...
    return MU_0 / (2 * np.pi) * (r - r_prime) / ((r - r_prime) ** 2 + (z - z_prime) ** 2)
